...
export default async function BlogPost({ params }: PageProps) {
  ...
  const date = page.data.date ? new Date(page.data.date as string) : new Date();
      ...
      <div className="flex divide-x divide-border relative max-w-7xl mx-auto px-4 md:px-0 z-10">
        ...
        <main className="w-full p-0 overflow-hidden">
          {page.data.thumbnail ? (
            <div className="relative w-full h-[500px] overflow-hidden object-cover border border-transparent">
              <Image
                src={page.data.thumbnail as string}
                alt={page.data.title}
                fill
        ...
        <aside className="hidden lg:block w-[350px] flex-shrink-0 p-6 lg:p-10 bg-muted/60 dark:bg-muted/20">
          <div className="sticky top-20 space-y-8">
            {page.data.author && typeof page.data.author === 'string' && isValidAuthor(page.data.author) ? (
